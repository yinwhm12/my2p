...
type Node struct {
	...
	config   *Config
	//accman   *accounts.Manager // todo new
...
func New(conf *Config) (*Node, error) {
	...
	//_, ephemeralKeystore, err := makeAccountManager(conf)  //todo new
	//if err != nil {
	//	return nil, err
	//}
	...
	return &Node{
		//accman:            am,// todo new
		//ephemeralKeystore: ephemeralKeystore,  //todo new
		config:            conf,
...
func (n *Node) Start() error {
	...
	for _, constructor := range n.serviceFuncs {
		...
		ctx := &ServiceContext{
			...
			services:       make(map[reflect.Type]Service),
			EventMux:       n.eventmux,
			//AccountManager: n.accman,// todo new
		}
...
//func (n *Node) AccountManager() *accounts.Manager { // todo new
//	return n.accman
//}
...
//func (n *Node) OpenDatabase(name string, cache, handles int) (ethdb.Database, error) {// todo new
//	if n.config.DataDir == "" {
	...
	return []rpc.API{
		{
			...
			Namespace: "debug",
			Version:   "1.0",
			//Service:   debug.Handler, // todo new
		}, {
			Namespace: "debug",
			Version:   "1.0",
